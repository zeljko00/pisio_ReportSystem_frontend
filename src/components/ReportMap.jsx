    ...
    <div className="mapArea">
      ...
      >
        ...
        {props.reports &&
          props.reports.map((r) => {
            r.type = t(r.type);
            return addMarkers(s + ": ", r, t("received"), t("closed"));
          })}
        ;
...
function addMarkers(s, report, date1, date2) {
  ...
  return (
    <Marker position={[report.x, report.y]} icon={markerIcon}>
      ;
      <Popup>
        <h3>{s + report.type}</h3>
        <h4>{date1 + " : " + report.date}</h4>
        <h4>{date2 + " : " + (report.solvedDate ? report.solvedDate : "-")}</h4>
      </Popup>
    </Marker>
  );
}
